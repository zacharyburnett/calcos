...
def test_get_columns(tmp_path):
    """
    Test if the function is returning the right column fields
    """
    # Setup
    test_data = generate_fits_file(tmp_path / "lbgu17qnq_corrtag_a.fits")
    dt = test_data[1].data
    detector = "FUV"
...
def test_remove_unwanted_column(tmp_path):
    """
    ...
    # Truth
    fd = generate_fits_file(tmp_path / "lbgu17qnq_lampflash.fits")
    table = fd[1].data
    cols = table.columns
...
def test_add_column_comment(tmp_path):
    # verify if entered comment to a header is present in the fits file.
    # Setup
    ofd = generate_fits_file(tmp_path / "myFitsFile.fits")
